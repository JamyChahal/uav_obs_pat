...
class CI(Controller):
    ...
    def random_index(self):
        index_x_des = random.randint(0, self.map_size_x * 2 * self.discretization)
        index_y_des = random.randint(0, self.map_size_y * 2 * self.discretization)
    ...
    def pose_callback(self, data):
        ...
        self.desired_closed_high_idleness()
        self.update_angle()
    ...
    def desired_closed_high_idleness(self):
        # Get the cells around us, near the observation range
        self.index_x_des, self.index_y_des = self.myMap.get_high_idleness_only_surrounding(self.x, self.y)

        self.x_des = (self.index_x_des / self.discretization) - self.map_size_x
        self.y_des = (self.index_y_des / self.discretization) - self.map_size_y
