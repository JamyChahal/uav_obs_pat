...
class Map_Env:
    ...
    def __init__(self, map_size_x, map_size_y, obs_range, discretization=1):

        self.map_size_x = map_size_x
        self.map_size_y = map_size_y

        self.obs_range = int(obs_range * discretization)
        ...
        self.map_idleness = np.zeros(self.discret_segment_size_x * self.discret_segment_size_y).astype(np.float32) * 100 #Init
        self.map_idleness.resize((self.discret_segment_size_x, self.discret_segment_size_y))

        self.map_targets = np.zeros(self.discret_segment_size_x * self.discret_segment_size_y).astype(np.float32)
        self.map_targets.resize((self.discret_segment_size_x, self.discret_segment_size_y))

        self.map_friends = copy.copy(self.map_targets)
        self.map_pose = copy.copy(self.map_targets)

        self.map_env = np.ones(self.discret_segment_size_x * self.discret_segment_size_y).astype(np.float32)
        self.map_env.resize((self.discret_segment_size_x, self.discret_segment_size_y))
    ...
    def next_time(self):
        ...
        self.map_targets -= 0.1
        self.map_friends -= 0.1
        self.map_pose -= 0.1
    ...
    def pose_transform(self, x_in, y_in):
        ret = True
        x, y = int((x_in + self.map_size_x) * self.discretization), int((y_in + self.map_size_y) * self.discretization)
        if not 0 <= x < self.map_size_x * 2 * self.discretization or not 0 <= y < self.map_size_y * 2 * self.discretization:
            ret = False
    ...
    def get_centered_map(self, x, y, mmap, default_outside_value=0.):
        ...
        _, x, y = self.pose_transform(x, y)
        #windows_radius = self.obs_range * 3
        windows_radius = 15
        ...
        x_max_check = x_max_w if x_max_w < self.discret_segment_size_x else self.discret_segment_size_x
        y_max_check = y_max_w if y_max_w < self.discret_segment_size_y else self.discret_segment_size_y
