...
namespace tello_driver
{
  ...
  VideoSocket::VideoSocket(TelloDriverNode *driver, std::string drone_ip, unsigned short video_port, const std::string &camera_info_path) :
    TelloSocket(driver, drone_ip, video_port)
  {
  ...
  void VideoSocket::process_packet(size_t r)
  {
    ...
    if (r < 1460) {
      //decode_frames();  //comment to stop processing images
